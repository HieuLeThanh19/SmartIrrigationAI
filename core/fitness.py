...
def penalty_water_under_target(x: np.ndarray, demand_max: np.ndarray, W_total: float) -> float:
    """Penalty khi chưa phân bổ hết lượng nước mục tiêu có thể dùng."""
    target = min(float(W_total), float(np.sum(demand_max)))
    return float(max(0.0, target - np.sum(x)) ** 2)
...
def fitness(x: np.ndarray, demand_min: np.ndarray,
    ...
    fitness = ALPHA * thiếu_nước
            + BETA * (dư_từng_thửa + vượt_tổng_nước)
            + DELTA * thiếu_so_với_mục_tiêu_nước
            + GAMMA * chi_phí_bơm
    """
    ...
    under_target = penalty_water_under_target(x, demand_max, W_total) if W_total is not None else 0.0
    cost     = pumping_cost(x, prices)
    return (
        cfg.ALPHA_PENALTY * shortage
        + cfg.BETA_WASTE * (waste + over_budget)
        + cfg.DELTA_UNDERUSE * under_target
        + cfg.GAMMA_COST * cost
    )
...
def fitness_breakdown(x: np.ndarray, demand_min: np.ndarray,
    ...
    underuse_penalty = cfg.DELTA_UNDERUSE * under_target
    cost_score = cfg.GAMMA_COST * cost
    total = shortage_penalty + waste_penalty + underuse_penalty + cost_score
    return {
